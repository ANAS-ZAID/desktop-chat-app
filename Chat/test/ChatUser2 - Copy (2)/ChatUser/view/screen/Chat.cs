...
namespace ChatUser.view.screen
{
    public partial class Chat : Form
    {
        ...
        private void AddMessageToChat(MessagesView message)
        {
            ...
            {
                Text = message.SentDate.HasValue?message.SentDate.Value.ToShortTimeString(): DateTime.Now.ToString("hh:mm tt"),
                AutoSize = true,
            ...
            if (message.isSender)
            {
                ...
                {
                    Name= "read",
                    Text = message.Status=="unread"? "⏳": message.Status =="sender" ? "✔️" : "✔️✔️",
                    AutoSize = true,
                    ForeColor = message.Status == "unread" ? Color.Black : message.Status == "sender" ? Color.Black : Color.Aqua,
                    ...
                    foreach (Control control in messagePanel.Controls)
                    {
                        ...
                        if (!id.HasValue||id==-1)
                        {
                            ...
                            if (read!=null)
                            {
                                ...
                                read.Text = message.Status == "unread" ? "⏳" : message.Status == "sender" ? "✔️" : "✔️✔️";

                               read. ForeColor = message.Status == "unread" ? Color.Black : message.Status == "sender" ? Color.Black : Color.Aqua;
                            }
                        }
