       ...
       // }
        ...
        public void Notification( JsonResponse notification)
        {
            ...
            lock (_clients)
            {
                ...
                if (!notification.Status&&!notification.isNotification) return;
                    ...
                    try
                    {
                        if (client.userId != null )
                        {
                            if (client.stream != null && (client.stream.CanWrite))
                            {
                                ...
                                lock (client)
                                {
                                    if (notification.Action == "newMessage")
                                    {

                                        var newMessage = notification.body?.FirstOrDefault();
                                        if (newMessage != null)
                                        {
                                            var message = new Messages() { json = newMessage };
                                            //if (client.userId == message.receiverPhone)
                                            //{
                                            message.status = "received";
                                            //}
                                            //MessageBox.Show($"newMessage:senderPhone,{message.senderPhone},receiverPhone:{message.receiverPhone}");
                                            if (client.userId == message.receiverPhone|| client.userId == message.senderPhone )
                                            {
                                                var chat = chatController.getChat(client.userId ?? 0, client.userId == message.senderPhone? message.receiverPhone:message.senderPhone);
                                                if (chat != null)
                                                {
                                                    //MessageBox.Show($"newMessage:senderPhone,{message.senderPhone},receiverPhone:{message.receiverPhone}");
                                                    //notification.body.Add(chat);
                                                    var header = new Dictionary<string, object>()
                                             {
                                              {"message",message.json },
                                              {"chat",chat },
                                            };
                                                    //var senderNotification = new JsonResponse() { Status = notification.Status, Action = notification.Action, Page = notification.Page, isNotification = true, header = header };
                                                    var senderNotification = notification.copy();
                                                    senderNotification.header = header;
                                                    senderNotification.Send(client.stream);
