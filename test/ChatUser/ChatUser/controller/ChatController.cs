    ...
    {       protected override string page => "chat";
       ...
       public void onClickChat(int index, Chat chat)
        ...
        public void selectChats(JsonResponse response)
        {
            ...
            if (response.header == null || !response.header.ContainsKey("allChats")) return;


            allChats = JsonConvert.DeserializeObject<List<Chat>>(response.header["allChats"].ToString());
        ...
        public List<Chat> search(string text)
        {
            if (string.IsNullOrEmpty(text)) return chats;
            return allChats.Where((chat) => ( chat.publicType&& (chat.userCode.ToString().Contains(text)|| chat.chatId.ToString().Contains(text))) || 
            (!chat.publicType && (chat.userCode==int.Parse(text)))).ToList();
